   ...
   <table width="100%" cellSpacing=0 cellPadding=0 border="0px">
	...
	<table class="query_form_table" cellSpacing=1 cellPadding=1>
		<tr>
			<td width=100% height=30 align=left
				background="images/m_table_top.jpg" colspan="6"><strong>&nbsp;类别信息>二级类别 </strong></td>
		</tr>
	...
	<table id="tb1" class="data_list_table" cellSpacing=1 cellPadding=1
		...
		<tr>
			<th>二级商品ID(不可修改)</th>
			<th>二级商品名称(单击可修改)</th>
			<th>所属一级商品ID(单击可修改)</th>
			<th>操作</th>
		</tr>
		
		<c:forEach var="secondclass" items="${requestScope.scl}">
			<tr>
				<td>${secondclass.getId() }</td>
				<td onclick="updatesectype('${secondclass.getId() }')">${secondclass.getType() }</td>
				<td onclick="updatesecfcid('${secondclass.getId() }')">${secondclass.getFc_id() }</td>
